...
public class UserInfo implements Serializable {
	...
	public String toString() {
		return "用户ID:" + (userId == null ? "空" : userId) + "," + 
				...
				"个人描述:" + (personDescription == null ? "空" : personDescription) + "," + 
				"加入时间:" + (joinTime == null ? "空" : DateUtils.format(joinTime, DateTimePatternEnum.YYYY_MM_DD_HH_SS.getPattern())) + "," + 
				"最后登录时间:" + (lastLoginTime == null ? "空" : DateUtils.format(lastLoginTime, DateTimePatternEnum.YYYY_MM_DD_HH_SS.getPattern())) + "," + 
				"最后登录IP:" + (lastLoginIp == null ? "空" : lastLoginIp) + "," + 
				"最后登录ip地址:" + (lastLoginIpAddress == null ? "空" : lastLoginIpAddress) + "," + 
				"积分:" + (totalIntegral == null ? "空" : totalIntegral) + "," + 
				"当前积分:" + (currentIntegral == null ? "空" : currentIntegral) + "," + 
				"0:禁用 1:正常:" + (status == null ? "空" : status);
	}
}